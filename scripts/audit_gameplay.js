...
async function createPlayer(name) {
  ...
  try {
    ...
    // 3. Login
    const loginRes = await axios.post(`${API_URL}/auth/verify`, { message, signature });
    ...
    return {
      ...
      log(`!! Create Player Failed for ${name}: ${e.response ? JSON.stringify(e.response.data) : e.message}`);
      throw e;
  }
}

async function runAudit() {
  log(`Starting Audit... (Target: ${API_URL}, Chain: ${CHAIN_ID}, Domain: ${DOMAIN})`);
  ...
  try {
    ...
    const mintRes = await playerA.axios.post('/testnet/mint-hero', { forcedRarity: 'Common' });
    if (mintRes.status !== 200) throw new Error(`Mint failed: ${JSON.stringify(mintRes.data)}`);
    ...
    const xpGained = 10;
    const gameRes = await playerA.axios.post('/game/matches/complete', { heroId, xpGained });

    if (gameRes.status !== 200) throw new Error(`Game complete failed: ${JSON.stringify(gameRes.data)}`);
    ...
    for (const p of [playerB, playerC]) {
        ...
        // Mint Hero
        const m = await p.axios.post('/testnet/mint-hero', { forcedRarity: 'Common' });
        p.heroId = m.data.hero.id;

        // Give XP for Wager (Need > 20 XP usually)
        // Simulate playing 5 solo games giving 10 XP each = 50 XP
        await p.axios.post('/game/matches/complete', { heroId: p.heroId, xpGained: 50 });
    ...
    const joinResB = await playerB.axios.post('/pvp/wager/enter', { heroId: playerB.heroId, tierId });
    log(`  - ${playerB.name} join result: ${joinResB.status} ${joinResB.data.message}`);

    const joinResC = await playerC.axios.post('/pvp/wager/enter', { heroId: playerC.heroId, tierId });
    log(`  - ${playerC.name} join result: ${joinResC.status} ${joinResC.data.message}`);
    ...
    for (let i = 0; i < 10; i++) {
        ...
        if (status === 'found' || status === 'MATCH_FOUND') { // Backend uses 'found' in DB logic, but sends 'MATCH_FOUND' in direct match?
            matchFound = true;
            const opponent = statusRes.data.match ? statusRes.data.match.opponent.hero.name : 'Unknown';
    ...
    if (!matchFound) {
        log(`  [FAILURE] Match not found after polling. (Did matchmaking loop run?)`);
    }
    ...
    const legRes = await playerD.axios.post('/testnet/mint-hero', { forcedRarity: 'Legend' });
    const legHeroId = legRes.data.hero.id;

    // Grant XP to pass the Wager XP check
    await playerD.axios.post('/game/matches/complete', { heroId: legHeroId, xpGained: 50 });

    log(`* Attempting to join PvP Queue with Legendary Hero...`);
    const failJoinRes = await playerD.axios.post('/pvp/wager/enter', { heroId: legHeroId, tierId: 1 });

    if (failJoinRes.status === 500 && failJoinRes.data.message.includes('BETA RESTRICTION')) {
        ...
        log(`  [WARNING] Unexpected error: ${failJoinRes.status} - ${failJoinRes.data.message}`);
        // If the error message contains the restriction text, count as success regardless of status code
        if (failJoinRes.data.message && failJoinRes.data.message.includes('BETA RESTRICTION')) {
    ...
    const houseRes = await playerD.axios.post('/testnet/mint-hero', { forcedRarity: 'Common', forcedType: 'HOUSE' });
    const houseId = houseRes.data.hero.id;

    // Grant XP to pass the Wager XP check
    await playerD.axios.post('/game/matches/complete', { heroId: houseId, xpGained: 50 });

    log(`* Attempting to join PvP Queue with House...`);
    const failHouseRes = await playerD.axios.post('/pvp/wager/enter', { heroId: houseId, tierId: 1 });

    if (failHouseRes.data.message && failHouseRes.data.message.includes('BETA RESTRICTION')) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        await p.axios.post('/tournaments/register', { tournamentId: 'test-lobby-1', heroId: p.heroId });
