...
function updateEnvFile(updates) {
  ...
  if (!fs.existsSync(envPath)) {
    console.warn(
      `[WARNING] .env file not found at ${envPath}. Creating a new one.`
    );
  ...
  console.log(
    '- Successfully updated the .env file with new contract addresses.'
  );
}
...
async function main() {
  ...
  if (!process.env.ORACLE_PRIVATE_KEY || !process.env.TEAM_WALLET_PRIVATE_KEY) {
    throw new Error(
      'ORACLE_PRIVATE_KEY and TEAM_WALLET_PRIVATE_KEY must be set in .env'
    );
  }
  ...
  console.log(
    `Minted ${ethers.formatUnits(initialMintAmount, 18)} BCOIN to deployer`
  );

  // 3. Deploy TournamentController
  const TournamentController = await ethers.getContractFactory(
  ...
  const PerpetualRewardPool = await ethers.getContractFactory(
    'PerpetualRewardPool'
  );
  ...
  const wagerArena = await WagerArena.deploy(bcoinAddress, oracle.address);
  ...
  const heroStaking = await HeroStaking.deploy(
    mockHeroNFTAddress,
    oracle.address
  ); // Pass NFT and Oracle addresses to constructor
  ...
  const tx1 = await tournamentController.setSoloRewardPool(
    perpetualRewardPoolAddress
  );
  await tx1.wait();
  ...
  const wagerTx2 = await wagerArena.setSoloRewardPool(
    perpetualRewardPoolAddress
  );
  await wagerTx2.wait();
  ...
  const tx2 = await bcoin.transfer(
    perpetualRewardPoolAddress,
    initialPoolFunding
  );
  await tx2.wait();
  ...
  console.log(
    `- Transferred ${ethers.formatUnits(
      initialPoolFunding,
      18
    )} BCOIN to the reward pool.`
  );
  console.log(
    `- PerpetualRewardPool BCOIN balance: ${ethers.formatUnits(
      poolBalance,
      18
    )}`
  );
  ...
  if (process.env.PRIVATE_KEY) {
    ...
    console.log(
      `- Transferred ${ethers.formatUnits(
        amountToTransfer,
        18
      )} BCOIN to test wallet ${testWalletAddress}.`
    );
    console.log(
      `- Test wallet BCOIN balance: ${ethers.formatUnits(
        testWalletBalance,
        18
      )}`
    );
  ...
  console.log(
    `- Transferred ${ethers.formatUnits(
      oracleBcoinAmount,
      18
    )} BCOIN to oracle ${oracle.address}.`
  );
  console.log(
    `- Oracle BCOIN balance: ${ethers.formatUnits(oracleBcoinBalance, 18)}`
  );
  ...
  console.log(
    `- Transferred ${ethers.formatEther(oracleEthAmount)} ETH to oracle ${
      oracle.address
    }.`
  );
  ...
  if (oracleEthBalance > 0 && oracleBcoinBalance > 0) {
    ...
    console.log(
      '[WARNING] Oracle wallet funding might have failed. Please check balances.'
    );
  }
  ...
  console.log(
    '- Wrote contract addresses to backend/contracts/contract-addresses.json'
  );
  ...
  const saveAbi = (contractIdentifier) => {
    ...
    console.log(
      `- Wrote ${contractName} ABI to backend/contracts/${contractName}.json`
    );
  };
