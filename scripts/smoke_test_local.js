...
async function run() {
  ...
  // Cleanup old db
  try {
    require('fs').unlinkSync('./smoke.sqlite');
  } catch (_) {
    // ignore
  }
  ...
  try {
    ...
    const user1Res = await request(
      'POST',
      '/api/debug/setup-test-player',
      { walletAddress: user1Addr },
      { 'x-admin-secret': ADMIN_SECRET }
    );

    if (!user1Res.body.success)
      throw new Error('User 1 setup failed: ' + JSON.stringify(user1Res.body));
    ...
    const token1 = jwt.sign(
      { userId: userId1, address: user1Addr },
      JWT_SECRET
    );
    ...
    const token2 = jwt.sign(
      { userId: userId2, address: user2Addr },
      JWT_SECRET
    );
    ...
    await request(
      'POST',
      '/api/debug/set-hero-xp',
      { heroId: hero1.id, xp: 100 },
      { 'x-admin-secret': ADMIN_SECRET }
    );
    await request(
      'POST',
      '/api/debug/set-hero-xp',
      { heroId: hero2.id, xp: 100 },
      { 'x-admin-secret': ADMIN_SECRET }
    );

    // 4. Test Matchmaking
    console.log('4. Entering Queue (User 1)...');
    const queue1 = await request(
      'POST',
      '/api/pvp/wager/enter',
    ...
    if (queue1.body.status !== 'QUEUED')
      throw new Error('User 1 should be QUEUED');

    console.log('5. Entering Queue (User 2)...');
    const queue2 = await request(
      'POST',
      '/api/pvp/wager/enter',
    ...
    const altarRes = await request(
      'POST',
      '/api/game/altar/donate',
      { txHash: '0x123', amount: '10' },
      { Authorization: `Bearer ${token1}` }
    );
    if (
      altarRes.body.success === false &&
      (altarRes.body.message.includes('failed') ||
        altarRes.body.message.includes('provider'))
    ) {
    ...
    const syncRes = await request('GET', '/api/cron/sync-staking', null, {
