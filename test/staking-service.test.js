...
describe.skip('StakingService', () => {
    ...
    beforeEach(() => {
        ...
        mockNftContract = {
            setApprovalForAll: sinon.stub().resolves({
                wait: sinon.stub().resolves({ status: 1, transactionHash: '0xapprovalhash' }),
            }),
        ...
        mockStakingContract = {
            depositHero: sinon.stub().resolves({
                wait: sinon.stub().resolves({ status: 1, transactionHash: '0xdeposithash' }),
            }),
        };

        // 4. Stub the ethers.Contract constructor to return our mocks
        const contractStub = sinon.stub(ethers, 'Contract');
        contractStub.withArgs(contractsConfig.heroStaking.address, sinon.match.any, mockSigner).returns(mockStakingContract);
        contractStub.withArgs(contractsConfig.mockHeroNFT.address, sinon.match.any, mockSigner).returns(mockNftContract);

        // 5. Stub the ethers.BrowserProvider to return a mock provider
        const mockProvider = {
            getSigner: sinon.stub().resolves(mockSigner),
        };
        browserProviderStub = sinon.stub(ethers, 'BrowserProvider').returns(mockProvider);
    ...
    it('should correctly call setApprovalForAll on the NFT contract', async () => {
        ...
        expect(mockNftContract.setApprovalForAll.calledWith(contractsConfig.heroStaking.address, true)).to.be.true;
    ...
    it('should check for approval status correctly', async () => {
        ...
        expect(mockNftContract.isApprovedForAll.calledWith('0xMockAddress', contractsConfig.heroStaking.address)).to.be.true;
    });
