    ...
    describe("Multi-Player Tournaments", function () {
        ...
        beforeEach(async function () {
            ...
            for (let i = 0; i < 7; i++) {
                ...
                await bcoin.connect(addrs[i]).approve(await tournamentController.getAddress(), TOURNAMENT_ENTRY_FEE);
            }
             await bcoin.connect(player1).approve(await tournamentController.getAddress(), TOURNAMENT_ENTRY_FEE);
             await bcoin.connect(player2).approve(await tournamentController.getAddress(), TOURNAMENT_ENTRY_FEE);
        });

        it("Should allow a player to create a 4-player tournament", async function () {
            await expect(tournamentController.connect(player1).createTournament(4, TOURNAMENT_ENTRY_FEE))
            ...
            expect(await bcoin.balanceOf(await tournamentController.getAddress())).to.equal(TOURNAMENT_ENTRY_FEE);
        });
        ...
        it("Should allow players to join an existing tournament and start it when full", async function () {
            ...
            await tournamentController.connect(player1).createTournament(4, TOURNAMENT_ENTRY_FEE);
            ...
            const participants = await tournamentController.getTournamentParticipants(1);
            expect(participants.length).to.equal(4);
            const totalPot = TOURNAMENT_ENTRY_FEE * BigInt(4);
            expect(await bcoin.balanceOf(await tournamentController.getAddress())).to.equal(totalPot);
        });
        ...
        it("Should fail if a player tries to join a full tournament", async function () {
            ...
            await expect(tournamentController.connect(addrs[2]).joinTournament(1))
