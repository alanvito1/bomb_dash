...
4.  Get the `DATABASE_URL` in **Project Settings** > **Database** > **Connection string (URI)**.
    - _Example:_ `postgresql://postgres:[PASSWORD]@db.project.supabase.co:5432/postgres`
...
2.  **Build Docker Image:**
    ...
    ```
    _This uses the `backend/` folder as the build context, ensuring the `Dockerfile` (inside it) finds files in the expected paths._
...
1.  **Create Service:**
    ...
    - **Authentication:** Allow unauthenticated invocations (public) - _Security is handled via JWT/App_.
...
2.  **Environment Variables:**
    ...
    _Tip: You can use GCP Secret Manager for better security._
