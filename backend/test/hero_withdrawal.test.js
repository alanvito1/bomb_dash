...
describe('Hero Withdrawal API Endpoint', () => {
  ...
  before(async () => {
    ...
    token = jwt.sign(
      { userId: userId, address: '0xTestUserWithdrawal' },
  ...
  it('should initiate withdrawal for a valid, staked hero', async () => {
    ...
    expect(res.body.message).to.equal(
      'Withdrawal signature generated successfully.'
    );
  ...
  it('should return 400 if the hero is not staked', async () => {
    ...
    expect(res.body.success).to.be.false;
    expect(res.body.message).to.equal(
      'This hero cannot be withdrawn. It must be a staked NFT.'
    );
  });
  ...
  it('should return 500 if the oracle signing fails', async () => {
    ...
    sinon
      .stub(oracle, 'signHeroWithdrawal')
      .rejects(new Error('Oracle signing failed'));
    ...
    expect(res.body.success).to.be.false;
    expect(res.body.message).to.equal(
      'Internal server error during withdrawal initiation.'
    );
  });
});
