...
describe('Staking Listener', () => {
    ...
    it('should handle cases where the hero is not found in the database gracefully', async () => {
        ...
        expect(consoleWarnSpy.calledWith(sinon.match(/no matching hero was found/))).to.be.true;
    ...
    it('should handle database errors without crashing', async () => {
        ...
        try {
            ...
            expect.fail(`handleHeroDeposited should not have thrown an error, but it threw: ${e.message}`);
        }

        // Verify that the error was logged to the console
        expect(consoleErrorSpy.calledWith(sinon.match(/Error updating database/), dbError)).to.be.true;
