...
describe('Integration Test: High-Stakes Wager Player Journey', () => {
  ...
  it('should de-level a hero and reset their XP to the max of the new level after a wager loss', async () => {
    ...
    const expectedXpForNewLevel =
      getExperienceForLevel(expectedNewLevel + 1) - 1; // 1000 - 1 = 999

    // Verification Critical: Assert the de-level occurred.
    expect(updatedLoserHero.level).to.equal(
      expectedNewLevel,
      'Hero did not de-level correctly to Level 4.'
    );

    // Verification Critical: Assert the XP was reset correctly for the new level.
    expect(updatedLoserHero.xp).to.equal(
      expectedXpForNewLevel,
      'Hero XP was not correctly adjusted to the maximum for the new level.'
    );
  });
});
