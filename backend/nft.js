...
const NFT_ABI = [
  ...
  },
  {
    ...
    name: 'design',
    outputs: [
      { internalType: 'contract IBHeroDesign', name: '', type: 'address' },
    ],
    stateMutability: 'view',
    type: 'function',
  },
  ...
  },
  {
    ...
    name: 'getTokenIdsByOwnerAndIndex',
    outputs: [{ internalType: 'uint256', name: '', type: 'uint256' }],
    stateMutability: 'view',
    type: 'function',
  },
...
function initNftService(provider, contractAddress) {
  if (!provider || !contractAddress) {
    throw new Error(
      'Provider and contract address are required to initialize the NFT service.'
    );
  }
  nftContract = new ethers.Contract(contractAddress, NFT_ABI, provider);
  console.log('[OK] NFT service initialized.');
}
...
async function getNftsForPlayer(playerAddress) {
  ...
  try {
    ...
    const decodedHeroes = packedDetailsArray.map((packedDetails) =>
      decodeHeroDetails(packedDetails)
    );
    ...
    console.error(
      `Failed to get NFTs for player ${playerAddress}:`,
      error.message
    );
...
async function assignMockHeroes(userId) {
  ...
  try {
    ...
    console.log(
      `Successfully created ${MOCK_HEROES.length} mock heroes for user ID: ${userId}.`
    );
