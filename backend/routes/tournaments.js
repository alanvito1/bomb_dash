...
router.get('/open', async (req, res) => {
    try {
        ...
        if (!contract) {
            return res.status(503).json({ success: false, message: "Tournament service is not available." });
        }
        ...
        res.status(500).json({ success: false, message: "Failed to fetch open tournaments." });
    }
});
...
router.post('/', async (req, res) => {
    ...
    if (!capacity || !entryFee || !txHash) {
        return res.status(400).json({ success: false, message: 'capacity, entryFee, and txHash are required.' });
    }
    ...
    try {
        ...
        res.json({ success: true, message: 'Tournament created successfully!', tournamentId });
    } catch (error) {
        console.error(`Error verifying tournament creation for user ${req.user.address}:`, error);
        res.status(400).json({ success: false, message: `Transaction verification failed: ${error.message}` });
    }
});
...
router.post('/:id/join', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, message: `Successfully joined tournament ${id}.` });
    } catch (error) {
        console.error(`Error verifying tournament join for user ${req.user.address}:`, error);
        res.status(400).json({ success: false, message: `Transaction verification failed: ${error.message}` });
    }
});
