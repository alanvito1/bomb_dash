...
function verifyAdmin(req, res, next) {
  ...
  if (!process.env.ADMIN_SECRET) {
      console.warn('ADMIN_SECRET not set in environment. Admin routes might be insecure or broken.');
  }

  if (adminSecret && adminSecret === (process.env.ADMIN_SECRET || 'avre-secret-key')) {
    next();
  } else {
    res.status(403).json({
      success: false,
...
router.get('/settings', async (req, res) => {
    try {
        const keys = ['levelUpCost', 'monsterScaleFactor', 'pvpWinXp', 'pvpCycleOpenHours', 'pvpCycleClosedHours', 'monsterXp', 'xp_multiplier'];
        const settings = {};
        ...
        if (settings.monsterXp) {
             try { settings.monsterXp = JSON.parse(settings.monsterXp); } catch(e) {}
        }
