...
router.post('/verify', async (req, res) => {
  ...
  if (!message || !signature) {
    return res.status(400).json({
      success: false,
      message: 'Requisição inválida: message e signature são obrigatórios.',
    });
  }
  ...
  try {
    ...
    if (siweMessage.domain !== process.env.FRONTEND_DOMAIN) {
      console.warn(
        `[Security] Blocked login attempt from invalid domain: ${siweMessage.domain} (Expected: ${process.env.FRONTEND_DOMAIN})`
      );
      return res
        .status(403)
        .json({ success: false, message: 'Domínio inválido.' });
    }

    if (siweMessage.chainId !== parseInt(process.env.CHAIN_ID)) {
      console.warn(
        `[Security] Blocked login attempt from invalid chain: ${siweMessage.chainId} (Expected: ${process.env.CHAIN_ID})`
      );
      return res
        .status(403)
        .json({ success: false, message: 'Chain ID inválido.' });
    }
    ...
    if (!expirationTime || Date.now() > expirationTime) {
      ...
      return res
        .status(403)
        .json({ success: false, message: 'Nonce inválido ou expirado.' });
    }
    ...
    if (!success) {
      return res.status(403).json({
        success: false,
        message: 'A verificação da assinatura falhou.',
      });
    }
    ...
    if (!user) {
      ...
      console.log(
        `First login for ${address}. Creating user and assigning heroes...`
      );
      ...
      if (userNfts && userNfts.length > 0) {
        console.log(
          `Found ${userNfts.length} NFT(s). Creating hero records for them.`
        );
