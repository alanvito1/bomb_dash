...
router.get('/', async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn(
        'NFT Sync failed (ignoring for Web2.5 fallback):',
        err.message
      );
    }
  ...
  try {
    ...
    if (user.coins < LEVEL_UP_BCOIN_COST) {
      return res
        ...
        .json({
          success: false,
          message: `Insufficient BCOIN. Need ${LEVEL_UP_BCOIN_COST}.`,
        });
    }
    ...
    if (!fragmentItem) {
      return res
        ...
        .json({
          success: false,
          message: 'System Error: Common Fragment not defined in economy.',
        });
    }
    ...
    if (!userFragment || userFragment.quantity < LEVEL_UP_FRAGMENT_COST) {
      return res
        ...
        .json({
          success: false,
          message: `Insufficient Common Fragments. Need ${LEVEL_UP_FRAGMENT_COST}.`,
        });
    }
    ...
    // Deduct Fragments
    const removeResult = await db.removeItemFromUser(
      userId,
      fragmentItem.id,
      LEVEL_UP_FRAGMENT_COST
    );
    if (!removeResult.success) {
      ...
      return res
        ...
        .json({
          success: false,
          message: 'Transaction Failed: Could not deduct fragments.',
        });
    }
