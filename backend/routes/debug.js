...
router.post('/assign-mock-hero', verifyAdmin, async (req, res) => {
  ...
  if (!walletAddress) {
    return res.status(400).json({
      success: false,
      message: 'O endereço da carteira (walletAddress) é obrigatório.',
    });
  }
  ...
  try {
    ...
    res.json({
      success: true,
      message: 'Heróis mock atribuídos com sucesso!',
      data: result,
    });
  } catch (error) {
    ...
    res.status(500).json({
      success: false,
      message: 'Erro interno ao atribuir o herói mock.',
    });
  }
});
...
router.post('/setup-test-player', verifyAdmin, async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[+] DEBUG 4: Fetching heroes for user ${user.id} to find one to stake...`
    );
    const heroes = await db.getHeroesByUserId(user.id);
    const mockHeroToStake = heroes.find((h) => h.hero_type === 'mock');
    console.log(
      `[+] DEBUG 4a: Found hero to stake: ${
        mockHeroToStake ? mockHeroToStake.id : 'None'
      }`
    );
    ...
    if (mockHeroToStake) {
      ...
      console.warn(
        `[!] DEBUG: Could not find a mock hero to stake for user ${user.id}.`
      );
    }
...
router.post('/set-hero-xp', verifyAdmin, async (req, res) => {
  ...
  if (!heroId || xp === undefined) {
    return res
      .status(400)
      .json({ success: false, message: 'heroId and xp are required.' });
  }
  ...
  try {
    ...
    res.json({
      success: true,
      message: `Hero ${heroId} XP set to ${xp}.`,
      hero,
    });
  } catch (error) {
