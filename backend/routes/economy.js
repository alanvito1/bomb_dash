...
router.post('/craft', verifyToken, async (req, res) => {
  try {
    ...
    if (!item1Id || !item2Id) {
      return res
        .status(400)
        .json({ success: false, message: 'Select items to craft.' });
    }
    ...
    if (user.coins < CRAFT_COST) {
      return res
        ...
        .json({
          success: false,
          message: `Insufficient BCOIN (${CRAFT_COST} required).`,
        });
    }
    ...
    if (item1Id === item2Id) {
      ...
      if (uItem1.quantity < 2) {
        return res
          .status(400)
          .json({ success: false, message: 'Need at least 2 items in stack.' });
      }
      ...
      if (
        !uItem1 ||
        !uItem2 ||
        uItem1.user_id !== userId ||
        uItem2.user_id !== userId
      ) {
        return res
          .status(400)
      ...
      if (uItem1.Item.name !== uItem2.Item.name) {
        return res
          .status(400)
          .json({ success: false, message: 'Items must be identical.' });
      }
    }
    ...
    if (!targetItemName) {
      return res
        .status(400)
        .json({ success: false, message: 'This item cannot be upgraded.' });
    }
    ...
    if (!targetItem) {
      return res
        .status(500)
        .json({ success: false, message: 'Target item definition missing.' });
    }
    ...
    // Update Reward Pool
    const pool = await GameSetting.findOne({
    ...
    if (success) {
      ...
      if (targetItem.rarity === 'Legendary' || targetItem.rarity === 'Mythic') {
        supabaseService.broadcastMessage('system-message', {
          message: `${user.wallet_address.substring(0, 6)}... crafted a ${
            targetItem.name
          }!`,
        });
      }
      ...
      res.json({
        ...
        message: `Success! Crafted ${targetItem.name}`,
      });
    } else {
      ...
      res.json({
        ...
        result: 'failure',
        message:
          'Critical Failure! Items lost, but you salvaged some Scrap Metal.',
      });
    }
  } catch (error) {
