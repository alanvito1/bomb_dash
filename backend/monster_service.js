...
function getMonsterStats(monsterBaseStats, playerLevel) {
  if (!monsterBaseStats || typeof playerLevel !== 'number' || playerLevel < 1) {
    throw new Error(
      'Invalid input for getMonsterStats. Provide base stats and a valid player level.'
    );
  }
  ...
  for (const stat in monsterBaseStats) {
    if (
      Object.hasOwnProperty.call(monsterBaseStats, stat) &&
      typeof monsterBaseStats[stat] === 'number'
    ) {
