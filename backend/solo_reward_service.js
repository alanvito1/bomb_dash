...
async function processRewardCycle() {
  ...
  try {
    ...
    console.log(
      `[SOLO REWARDS] Found ${gamesInLastCycle} solo games in the last cycle (since ${lastCycleStartTime.toISOString()}).`
    );
    ...
    console.error(
      '[SOLO REWARDS] An error occurred during reward cycle processing:',
      error.message
    );
  }
}
...
function startSoloRewardCycleCron() {
  ...
  setTimeout(async () => {
    try {
      console.log(
        '[SOLO REWARDS] Performing initial reward cycle run on startup...'
      );
    ...
    try {
      ...
      if (
        ...
        error.message.includes(
          'A new cycle can only be started every 10 minutes'
        )
      ) {
        console.warn(
          `[SOLO REWARDS] Attempted to start a new cycle too early. Will try again in 10 minutes.`
        );
      } else {
        console.error(
          '[CRON-FATAL] Unhandled exception in solo reward cycle cron job:',
          error
        );
      }
    }
