...
if (!supabaseUrl || !supabaseKey) {
  console.error('CRITICAL: Supabase URL or Service Role Key missing from environment variables.');
...
async function seedEconomy() {
  ...
  const fragments = [
    { name: 'Common Fragment', type: 'fragment', rarity: 'Common', stats: {}, image_url: 'fragment_common' },
    { name: 'Rare Fragment', type: 'fragment', rarity: 'Rare', stats: {}, image_url: 'fragment_rare' },
    { name: 'Super Rare Fragment', type: 'fragment', rarity: 'Super Rare', stats: {}, image_url: 'fragment_super_rare' },
    { name: 'Epic Fragment', type: 'fragment', rarity: 'Epic', stats: {}, image_url: 'fragment_epic' },
    { name: 'Legendary Fragment', type: 'fragment', rarity: 'Legendary', stats: {}, image_url: 'fragment_legendary' },
    { name: 'Super Legendary Fragment', type: 'fragment', rarity: 'Super Legendary', stats: {}, image_url: 'fragment_super_legendary' }
  ];
  ...
  for (const fragment of fragments) {
    ...
    if (!data) {
        ...
        if (insertError) {
            console.error(`Failed to create ${fragment.name}:`, insertError.message);
        }
    }
  }
...
async function getHeroesByUserId(userId) {
  ...
  return data.map(h => ({
      ...
      // Name Logic
      name: h.sprite_name
        ? h.sprite_name.split('_').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')
        : 'Unknown Hero'
  }));
}
...
async function addXpToHero(heroId, xpAmount) {
    ...
    if (fetchError || !hero) throw new Error(`Hero not found: ${fetchError?.message}`);
...
async function savePlayerCheckpoint(userId, waveNumber) {
    ...
    if (fetchError) throw new Error(`Fetch Checkpoint Failed: ${fetchError.message}`);
    ...
    if (existing) {
        if (waveNumber > existing.highest_wave_reached) {
            const { error: updateError } = await supabase
                ...
                .eq('user_id', userId);
            if (updateError) throw new Error(`Update Checkpoint Failed: ${updateError.message}`);
        ...
        if (insertError) throw new Error(`Create Checkpoint Failed: ${insertError.message}`);
        return { success: true, updated: true };
    }
}
...
async function getGameSetting(key) {
    const { data } = await supabase.from('game_settings').select('value').eq('key', key).maybeSingle();
...
async function addItemToUser(userId, itemId, quantity = 1) {
    ...
    if (fetchError) throw new Error(`Fetch User Item Failed: ${fetchError.message}`);
    ...
    if (existing) {
        const { error: updateError } = await supabase
            ...
            .eq('id', existing.id);
        if (updateError) throw new Error(`Update Item Quantity Failed: ${updateError.message}`);
    } else {
        const { error: insertError } = await supabase
            .from('user_items')
...
async function grantRewards(userId, bcoinReward, accountXpReward) {
    ...
    if (updateError) throw new Error(`Grant Rewards Failed: ${updateError.message}`);
    return { success: true, user: { ...user, coins: newCoins, account_xp: newXp, account_level: newLevel } };
}
...
async function updateBestiary(userId, updates) {
    for (const [enemyType, count] of Object.entries(updates)) {
        ...
        if (existing) {
            ...
            await supabase
                .from('user_bestiary')
                .insert([{ user_id: userId, enemy_type: enemyType, kill_count: count }]);
        }
    }
    return { success: true };
}
...
async function updateHeroProficiency(heroId, updates) {
    ...
    const updatePayload = {};
    if (updates.bombMasteryXp) updatePayload.bomb_mastery_xp = (hero.bomb_mastery_xp || 0) + updates.bombMasteryXp;
    if (updates.agilityXp) updatePayload.agility_xp = (hero.agility_xp || 0) + updates.agilityXp;
...
async function addDonationToAltar(amount, txHash) {
    const { data: altar } = await supabase.from('altar_status').select('*').eq('id', 1).maybeSingle();
    let currentDonations = 0;
    if (altar) currentDonations = altar.current_donations;
    else await supabase.from('altar_status').insert([{id: 1, current_donations: 0}]);

    await supabase.from('altar_donations').insert([{ tx_hash: txHash, amount }]);
    await supabase.from('altar_status').update({ current_donations: currentDonations + amount }).eq('id', 1);
    return { success: true };
}
...
async function getWagerTier() { return null; }
async function getWagerTiers() { return []; }
async function processWagerMatchResult() { return { success: false, message: "PvP disabled in Refactor" }; }
async function createWagerMatch() { return { success: false }; }
async function getWagerMatch() { return null; }
async function updateWagerMatch() { return { success: false }; }
async function addToMatchmakingQueue() { return { success: false }; }
async function removeFromMatchmakingQueue() { return { success: true }; }
async function getMatchmakingQueueUser() { return null; }
async function updateMatchmakingQueueStatus() { return { success: true }; }
