...
async function joinQueue(userId, heroId, tier = 'default') {
    ...
    console.log(`[Matchmaking] User ${userId} with hero ${heroId} joined the queue for tier ${tier}.`);
...
async function findOpponent(player, queue) {
    ...
    const potentialOpponents = queue.filter(o => o.tier === player.tier && o.userId !== player.userId && o.status === 'searching');
    ...
    for (const opponent of potentialOpponents) {
        ...
        if (levelDifference <= levelRange) {
            console.log(`[Matchmaking] Match found for User ${player.userId} (Lvl ${playerHero.level}) and User ${opponent.userId} (Lvl ${opponentHero.level}) in tier ${player.tier}`);
...
async function processQueue() {
    try {
        ...
        for (const player of searchingPlayers) {
            ...
            if (!match) { // If no human opponent was found
            ...
            if (match) {
                ...
                console.log(`[Matchmaking] Match created for User ${player1Id} against ${player2Id === -1 ? 'Test Bot' : 'User ' + player2Id}`);
                ...
                if (player1InQueue) {
                    ...
                    player1InQueue.match = { opponent: match.player2, tier: player1InQueue.tier };
                }
                ...
                if (player2Id !== -1) {
                    ...
                    if (player2InQueue) {
                        ...
                        player2InQueue.match = { opponent: match.player1, tier: player2InQueue.tier };
                    }
                }
...
async function checkBotMatch(player) {
    ...
    if (waitTimeSeconds > botTimeout) {
        console.log(`[Matchmaking] Player ${player.userId} in tier ${player.tier} has waited ${waitTimeSeconds.toFixed(1)}s. Creating a bot match.`);

        const playerHero = (await db.getHeroesByUserId(player.userId)).find(h => h.id === player.heroId);
        if (!playerHero) {
            // This should not happen if the player is in the queue, but as a safeguard:
            console.error(`[Matchmaking] CRITICAL: Could not find hero ${player.heroId} for player ${player.userId} for bot match.`);
