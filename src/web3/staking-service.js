...
async function getEthersDependencies() {
  ...
  try {
    ...
    throw new Error(
      'Could not connect to wallet or contracts. ' + error.message
    );
  }
}
...
class StakingService {
  ...
  async approve() {
    ...
    const txPromise = nftContract.setApprovalForAll(
      contracts.heroStaking.address,
      true
    );
    await this._handleTransaction(
      txPromise,
      'Approval successful. You can now stake your heroes.'
    );
  }
  ...
  async withdrawHero(tokenId, level, xp, signature) {
    const { stakingContract } = await getEthersDependencies();
    const txPromise = stakingContract.withdrawHero(
      tokenId,
      level,
      xp,
      signature
