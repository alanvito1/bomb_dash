...
export default class HUD {
    ...
    createStatusBars(playerStats) {
        ...
        this.healthText = this.scene.add.text(130, 10, '', { fontSize: '14px', fill: '#ffffff', align: 'center' }).setOrigin(0.5, 0);
    ...
    createTargetFrame() {
        this.targetFrame = this.scene.add.container(this.scene.scale.width / 2, 30).setVisible(false);
        const bg = this.scene.add.graphics().fillStyle(0x000000, 0.5).fillRect(-100, 0, 200, 40);
        this.targetNameText = this.scene.add.text(0, 5, 'Enemy Name', { fontSize: '14px', fill: '#ffffff' }).setOrigin(0.5, 0);
    ...
    createActionBar(width, height) {
        ...
        for (let i = 0; i < numSlots; i++) {
            ...
            const slot = this.scene.add.graphics().fillStyle(0x000000, 0.5).fillRect(x, y, slotSize, slotSize);
            this.actionBarSlots.push(slot);
        }
    }

    createChatLog(height) {
        const chatBg = this.scene.add.graphics().fillStyle(0x000000, 0.4).fillRect(10, height - 150, 300, 140);
    ...
    createPvpStatusDisplay(width) {
        ...
        this.pvpCountdownText = this.scene.add.text(width - 40, 80, '', {
            fontSize: '12px',
            fill: '#ffff00',
            align: 'center'
    ...
    showTarget(enemy) {
        ...
        this.targetHealthBar.clear().fillStyle(0xff0000, 1).fillRect(-98, 25, 196 * healthPercentage, 10);
    }
    ...
    updateGlobalBuffs(buffs = []) {
        ...
        buffs.forEach((buff, index) => {
            ...
            for (let i = 0; i < 5; i++) {
                icon.lineTo(x + Math.cos(rot) * outerRadius, y + Math.sin(rot) * outerRadius);
                rot += step;
                icon.lineTo(x + Math.cos(rot) * innerRadius, y + Math.sin(rot) * innerRadius);
                rot += step;
            }
            ...
            // Tooltip em hover
            icon.setInteractive(new Phaser.Geom.Circle(16, 16, 16), Phaser.Geom.Circle.Contains);
            icon.on('pointerover', () => {
                this.showTooltip(buff.name, buff.effect, buff.duration, this.scene.input.activePointer.x, this.scene.input.activePointer.y);
            });
            icon.on('pointerout', () => {
                this.hideTooltip();
            });
    ...
    showTooltip(name, effect, duration, x, y) {
        ...
        this.tooltipBg = this.scene.add.graphics().fillStyle(0x000000, 0.8).setDepth(101);
        this.tooltipText = this.scene.add.text(x, y - 50, content, {
            fontSize: '12px',
            fill: '#ffffff',
            align: 'center'
        ...
        this.tooltipBg.fillRect(textBounds.x - 5, textBounds.y - 5, textBounds.width + 10, textBounds.height + 10);
    }
    ...
    updatePowerupDisplay(activePowerups) {
        ...
        for (const [id, powerup] of Object.entries(activePowerups)) {
            ...
            this.powerupIcons[id] = this.scene.add.image(baseX, baseY, id)
                .setDisplaySize(32, 32).setOrigin(0);

            this.powerupTimers[id] = this.scene.add.text(baseX + 16, baseY + 35, `${Math.ceil(powerup.remaining / 1000)}s`, {
                fontSize: '12px',
                fill: '#ffff00',
