...
export default class TournamentBracketScene extends Phaser.Scene {
  ...
  async fetchAndDisplayBracket() {
    ...
    try {
      ...
      if (response.success && response.tournament) {
        ...
        this.add
          .text(centerX, 200, 'Could not load tournament bracket.', {
      ...
      this.add
        .text(centerX, 200, 'Error loading bracket.', {
          fontFamily: '"Press Start 2P"',
          fontSize: '16px',
          fill: '#ff0000',
        })
  ...
  displayBracket(tournament) {
    ...
    this.add
      .text(centerX, startY, 'Participants:', {
        fontFamily: '"Press Start 2P"',
        fontSize: '14px',
        fill: '#ffffff',
      })
    ...
    tournament.participants.forEach((participant, index) => {
      ...
      const shortAddress = `${participant.substring(
        0,
        8
      )}...${participant.substring(participant.length - 6)}`;
      this.add
        .text(centerX, y, shortAddress, {
          fontFamily: '"Press Start 2P"',
          fontSize: '12px',
          fill: '#cccccc',
        })
    ...
    tournament.rounds.forEach((round, roundIndex) => {
      this.add
        .text(centerX, roundY, `--- Round ${roundIndex + 1} ---`, {
          fontFamily: '"Press Start 2P"',
          fontSize: '14px',
          fill: '#ffffff',
        })
      ...
      round.forEach((match) => {
        ...
        const winner = match.winner
          ? `-> Winner: ${match.winner.substring(0, 6)}..`
          : '';
        this.add
          .text(centerX, roundY, `${player1} vs ${player2} ${winner}`, {
            fontFamily: '"Press Start 2P"',
            fontSize: '12px',
            fill: '#cccccc',
          })
