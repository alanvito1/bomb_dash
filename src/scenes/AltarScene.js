...
export default class AltarScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      ...
      .fillRect(30, 30, this.scale.width - 60, this.scale.height - 60);
    ...
    // --- UI Elements ---
    this.add
      .text(centerX, 80, LanguageManager.get('altar_title'), titleStyle)
      .setOrigin(0.5);
    ...
    this.statusText = this.add
      .text(
        centerX,
        150,
        LanguageManager.get('altar_fetching_status'),
        textStyle
      )
      .setOrigin(0.5);
    ...
    this.add
      .text(
        centerX,
        centerY - 20,
        LanguageManager.get('altar_donate_prompt'),
        textStyle
      )
      .setOrigin(0.5);

    this.donationInput = this.add
    ...
    const donateButton = this.add
      .text(centerX, centerY + 80, LanguageManager.get('altar_donate_button'), {
        ...buttonStyle,
        fill: '#FFD700',
      })
      .setOrigin(0.5)
    ...
    const backButton = this.add
      .text(
        ...
        LanguageManager.get('back_to_menu'),
        buttonStyle
      )
      .setOrigin(0.5)
      .setInteractive({ useHandCursor: true });
  ...
  updateStatusText(status) {
    ...
    let text = LanguageManager.get('altar_status_goal', {
      donated: current_donations,
      goal: donation_goal,
    });

    if (active_buff_type) {
      const expires = new Date(buff_expires_at).toLocaleString();
      text += LanguageManager.get('altar_status_buff', {
        buff: active_buff_type,
        expires: expires,
      });
    } else {
  ...
  async handleDonation() {
    ...
    try {
      ...
      const bcoinContract = new ethers.Contract(
        contracts.bcoin.address,
        contracts.bcoin.abi,
        signer
      );
      const amountInWei = ethers.parseUnits(amount.toString(), 18);
      const approveTx = await bcoinContract.approve(
      ...
      this.showToast(
        LanguageManager.get('altar_error_connection') + `: ${error.message}`
      );
    }
  }
