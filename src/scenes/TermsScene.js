...
class TermsScene extends Phaser.Scene {
  ...
  createUI() {
    ...
    this.add
      .text(centerX, 50, LanguageManager.get('terms.title'), {
    ...
    graphics
      ...
      .strokeRect(centerX - 220, scrollableAreaY, 440, scrollableAreaHeight);
    ...
    this.maxScroll = Math.max(
      0,
      this.textObject.height - (scrollableAreaHeight - 20)
    );
    ...
    if (!this.canAccept) {
      ...
      this.input.on('pointerdown', (pointer) => {
        if (
          pointer.y >= scrollableAreaY &&
          pointer.y <= scrollableAreaY + scrollableAreaHeight
        ) {
          isDragging = true;
    ...
    this.add
      .text(
        centerX,
        scrollableAreaY + scrollableAreaHeight + 15,
        LanguageManager.get('terms.scroll_prompt'),
        {
          fontFamily: 'Arial',
          fontSize: '12px',
          fill: '#888888',
