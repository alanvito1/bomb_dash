...
export default class PvpScene extends Phaser.Scene {
  ...
  create() {
    ...
    const backButton = this.add
      .text(centerX, this.scale.height - 50, 'Voltar', {
    ...
    const buttonY = 120;
    this.rankedButton = this.createModeButton(
      centerX - 150,
      buttonY,
      LanguageManager.get('pvp_ranked'),
      'ranked'
    );
    this.wagerButton = this.createModeButton(
      centerX,
      buttonY,
      LanguageManager.get('pvp_wager'),
      'wager'
    );
    this.botButton = this.createModeButton(
      centerX + 150,
      buttonY,
      LanguageManager.get('pvp_bot'),
      'bot'
    );
  ...
  async drawWagerUI() {
    ...
    const loadingText = this.add
      .text(this.game.config.width / 2, 300, 'Carregando tiers...', {
        fontFamily: '"Press Start 2P"',
        fontSize: '24px',
        color: '#FFFFFF',
      })
    ...
    try {
      ...
      this.heroSelectionContainer = this.add
        .container(0, 180)
        .setVisible(false);
      this.contentContainer.add(this.heroSelectionContainer);

      const promptText = this.add
        .text(this.game.config.width / 2, 0, 'Selecione um Tier de Aposta', {
          fontFamily: '"Press Start 2P"',
          fontSize: '20px',
          color: '#FFFFFF',
        })
  ...
  createTierCard(x, y, tier) {
    ...
    const tierName = this.add.text(-305, -30, tier.name, {
      fontFamily: '"Press Start 2P"',
      fontSize: '24px',
      color: '#FFD700',
    });
    const bcoinCost = this.add.text(
      -305,
      15,
      `Aposta: ${tier.bcoin_cost} BCOIN`,
      { fontFamily: '"Press Start 2P"', fontSize: '14px', color: '#00FFFF' }
    );
    const xpCost = this.add.text(80, 15, `Risco: ${tier.xp_cost} Hero XP`, {
      fontFamily: '"Press Start 2P"',
      fontSize: '14px',
      color: '#FF69B4',
    });
  ...
  displayHeroesForWager() {
    ...
    const promptText = this.add
      .text(
        this.game.config.width / 2,
        0,
        `Selecione um Herói (Risco: ${this.selectedTier.xp_cost} XP)`,
        {
          fontFamily: '"Press Start 2P"',
          fontSize: '18px',
          color: '#FFFFFF',
    ...
    const backToTiersButton = this.add
      .text(
        this.game.config.width / 2,
  ...
  createHeroCard(x, y, hero, isSelectable) {
    ...
    const heroName = this.add.text(
      -230,
      -25,
      `${hero.sprite_name} (Lvl: ${hero.level})`,
      { fontFamily: '"Press Start 2P"', fontSize: '20px', color: textColor }
    );
    const heroXp = this.add.text(-230, 10, `XP Atual: ${hero.xp}`, {
      fontFamily: '"Press Start 2P"',
      fontSize: '14px',
      color: textColor,
    });
  ...
  showEnterWagerButton() {
    ...
    this.enterWagerButton = this.add
      .text(centerX, buttonY, buttonText, {
        fontFamily: '"Press Start 2P"',
        fontSize: '18px',
        fill: '#00ff00',
  ...
  async handleEnterWagerQueue() {
    ...
    this.showPopup(
      'Processando... Por favor, aprove as transações na sua carteira.'
    );

    try {
      // 1. Approve BCOIN transfer
      const bcoinContract = new this.web3.eth.Contract(
        contracts.bcoin.abi,
        contracts.bcoin.address
      );
      const wagerCost = this.web3.utils.toWei(
        this.selectedTier.bcoin_cost.toString(),
        'ether'
      );

      this.showPopup('Passo 1/2: Aprovando aposta em BCOIN...');
      await bcoinContract.methods
        .approve(contracts.wagerArena.address, wagerCost)
      ...
      const wagerContract = new this.web3.eth.Contract(
        contracts.wagerArena.abi,
        contracts.wagerArena.address
      );
      const receipt = await wagerContract.methods
        .enterWagerQueue(this.selectedTier.id)
        .send({ from: this.userData.address });
  ...
  listenForWagerMatch() {
    ...
    this.eventListener = wagerContract.events
      .WagerMatchCreated({ filter: { tierId: this.selectedTier.id } })
      .on('data', (event) => {
        const { player1, player2 } = event.returnValues;
        const myAddress = this.userData.address.toLowerCase();

        if (
          player1.toLowerCase() === myAddress ||
          player2.toLowerCase() === myAddress
        ) {
  ...
  goToGameScene(matchData) {
    const opponentAddress =
      matchData.player1.toLowerCase() === this.userData.address.toLowerCase()
        ? matchData.player2
        : matchData.player1;
  ...
  showEnterQueueButton() {
    ...
    this.enterQueueButton = this.add
      .text(
        this.game.config.width / 2,
    ...
    this.enterQueueButton.on('pointerover', () =>
      this.enterQueueButton.setStyle({ fill: '#ffffff' })
    );
    this.enterQueueButton.on('pointerout', () =>
      this.enterQueueButton.setStyle({ fill: '#00ff00' })
    );
  }

  async handleEnterQueue() {
    this.showPopup(
      'Processando taxa de entrada... Por favor, aprove a transação na sua carteira.'
    );
    ...
    try {
      ...
      await bcoinContract.methods
        .approve(contracts.tournamentController.address, feeInWei)
        .send({ from: this.userData.address });

      this.showPopup(
        'Aprovação bem-sucedida! Agora, confirme a transação para entrar na fila.'
      );

      const tournamentContract = new this.web3.eth.Contract(
        contracts.tournamentController.abi,
        contracts.tournamentController.address
      );
      const tier = this.selectedHero.level;

      const receipt = await tournamentContract.methods
        .enterRankedMatch(tier, feeInWei)
        .send({ from: this.userData.address });
      ...
      this.showPopup(
        `Falha ao entrar na fila: ${error.message.substring(0, 50)}...`
      );
    }
  }
  ...
  startMatchmakingStatusCheck() {
    if (this.enterQueueButton)
      this.enterQueueButton.setInteractive(false).setStyle({ fill: '#888888' });
  ...
  showPopup(message) {
    ...
    const bg = this.add.graphics({
      fillStyle: { color: 0x000000, alpha: 0.9 },
    });
