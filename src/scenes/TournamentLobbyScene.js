...
export class TournamentLobbyScene extends Phaser.Scene {
    ...
    create() {
        ...
        createButton(this, centerX, this.cameras.main.height - 100, LanguageManager.get('menu_back'), () => {
    ...
    async fetchAndDisplayTournaments() {
        ...
        try {
            ...
            if (response.success && response.tournaments.length > 0) {
                ...
                this.add.text(centerX, 200, 'No open tournaments found.', {
            ...
            this.add.text(centerX, 200, 'Error loading tournaments.', {
                fontFamily: '"Press Start 2P"',
                fontSize: '16px',
                fill: '#ff0000'
            }).setOrigin(0.5);
    ...
    async joinTournament(tournamentId, entryFee) {
        ...
        try {
            ...
            const response = await api.post(`/tournaments/${tournamentId}/join`, { txHash: fakeTxHash });

            if (response.success) {
                console.log(`Successfully joined tournament ${tournamentId}`);
                this.scene.start(CST.SCENES.TOURNAMENT_BRACKET, { tournamentId: tournamentId });
            } else {
