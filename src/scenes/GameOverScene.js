...
export default class GameOverScene extends Phaser.Scene {
  ...
  create(data) {
    ...
    this.add.text(centerX, centerY - 200, LanguageManager.get('game_over_title'), {
        ...
        stroke: '#000000',
        strokeThickness: 6,
        shadow: { offsetX: 3, offsetY: 3, color: '#ff0000', blur: 15, stroke: true, fill: true }
    }).setOrigin(0.5);

    // --- Stats Display ---
    const statsY = centerY - 100;
    const textStyle = { fontFamily: '"Press Start 2P"', fontSize: '18px', fill: '#ffffff' };
    ...
    // XP Gained
    this.add.text(centerX, statsY + 80, `XP Gained: ${xpGained}`, valueStyle).setOrigin(0.5);

    // Coins Earned
    this.add.text(centerX, statsY + 120, `${LanguageManager.get('game_over_coins_earned', { coins: '' })}${coinsEarned}`, valueStyle).setOrigin(0.5);
    ...
    // Menu Button
    const menuButton = this.add.image(centerX + buttonSpacing / 2, buttonY, 'btn_menu').setInteractive({ useHandCursor: true });
    this.add.text(menuButton.x, menuButton.y, LanguageManager.get('menu_title', {}, 'Menu'), textStyle).setOrigin(0.5);
