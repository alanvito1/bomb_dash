...
export default class ShopScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.add
      ...
      .fillRect(20, 20, this.scale.width - 40, this.scale.height - 40);

    // --- Guard Clause ---
    if (!this.hero) {
      this.add
        .text(centerX, centerY, LanguageManager.get('shop_hero_not_found'), {
    ...
    this.add
      .text(centerX, 60, LanguageManager.get('shop_title'), titleStyle)
      .setOrigin(0.5);

    this.coinsText = this.add
      .text(centerX, 100, LanguageManager.get('shop_coins', { coins: '...' }), {
        ...textStyle,
        fill: '#FFD700',
      })
      .setOrigin(0.5);
  ...
  createStatDisplays(x, y, style) {
    const statGetters = [
      ...
      () =>
        LanguageManager.get('shop_stat_fire_rate', {
          value: this.hero.fireRate,
        }),
    ];
  ...
  createUpgradeButtons(x, y, style) {
    ...
    this.upgradeButtons = buttonsConfig.map((btnConfig, i) => {
      ...
      const label = () =>
        LanguageManager.get(`shop_upgrade_${btnConfig.type}`, { cost });

      const button = this.add
        .text(x, buttonY, label(), style)
        .setOrigin(0.5)
      ...
      button.on('pointerdown', async () => {
        ...
        try {
          ...
          button.setText(
            LanguageManager.get('shop_verifying', {}, 'Verifying...')
          );
          const result = await api.updateUserStats(
            this.hero.id,
            btnConfig.type,
            payTx.hash
          );
  ...
  handleBalanceUpdate({ balance, error }) {
    ...
    this.coinsText.setText(
      LanguageManager.get('shop_coins', { coins: balanceFormatted })
    );
  }
  ...
  refreshUI() {
    if (!this.hero) return;
    this.statTexts[0].setText(
      LanguageManager.get('shop_stat_damage', { value: this.hero.damage })
    );
    this.statTexts[1].setText(
      LanguageManager.get('shop_stat_speed', { value: this.hero.speed })
    );
    this.statTexts[2].setText(
    ...
    this.upgradeButtons.forEach((btn) => {
      ...
      const newLabel = LanguageManager.get(`shop_upgrade_${btn.config.type}`, {
        cost: newCost,
      });
