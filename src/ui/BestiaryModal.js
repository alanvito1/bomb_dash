  ...
  createEnemySlot(x, y, size, mob, kills) {
    ...
    if (this.scene.textures.exists(assetKey)) {
      ...
      const scale = Math.min(
        maxIconSize / icon.width,
        maxIconSize / icon.height
      );
      icon.setScale(scale);
      ...
      const text = this.scene.add
        .text(size / 2, size / 2, '?', { fontSize: '24px', fill: '#666' })
        .setOrigin(0.5);
      container.add(text);
    }
    ...
    const nameText = this.scene.add
      .text(size / 2, 10, isDiscovered ? mob.name : '???', {
    ...
    const barBg = this.scene.add
      .rectangle(barX, barY, barWidth, barHeight, 0x000000)
      .setOrigin(0);
    const barColor = isCompleted ? 0xffd700 : 0x00ff00;
    const barFill = this.scene.add
      .rectangle(barX, barY, barWidth * progress, barHeight, barColor)
      .setOrigin(0);

    container.add([barBg, barFill]);

    // Kills Text
    const killsText = this.scene.add
      .text(size / 2, size - 5, `${kills}/${target}`, {
        fontFamily: '"Press Start 2P"',
        fontSize: '8px',
        fill: '#ffffff',
      })
