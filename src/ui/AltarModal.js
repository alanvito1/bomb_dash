...
export default class AltarModal extends UIModal {
    constructor(scene) {
        super(scene, 400, 500, LanguageManager.get('altar_title', {}, 'ALTAR OF POWER'));
        this.populate();
    }
    ...
    populate() {
        ...
        if (this.scene.textures.exists('icon_altar')) {
            const icon = this.scene.add.image(cx, startY + 40, 'icon_altar').setScale(2);
            ...
            const text = this.scene.add.text(cx, startY + 40, 'ALTAR', { fontSize: '12px', fill: '#000' }).setOrigin(0.5);
        ...
        // Label
        const progressLabel = this.scene.add.text(cx, progressY - 20, 'GLOBAL GOAL: 50,000 BCOIN', {
        ...
        // Bar Bg
        const barBg = this.scene.add.rectangle(cx, progressY, barW, barH, 0x333333).setOrigin(0.5);
        // Bar Fill (Mock 60%)
        const fillW = barW * 0.6;
        const barFill = this.scene.add.rectangle(cx - barW / 2 + fillW / 2, progressY, fillW, barH, 0xffd700).setOrigin(0.5);

        this.windowContainer.add([barBg, barFill]);

        // 3. Current Buff
        const buffY = progressY + 60;
        const buffText = this.scene.add.text(cx, buffY, 'CURRENT BUFF:\n+10% XP (Ends in 2h)', {
            fontFamily: '"Press Start 2P"',
            fontSize: '14px',
            fill: '#00ff00',
            align: 'center'
        ...
        const btnLabel = this.scene.add.text(0, 0, 'DONATE 100 BCOIN', {
            fontFamily: '"Press Start 2P"',
            fontSize: '14px',
            fill: '#000000'
        }).setOrigin(0.5);
    ...
    async handleDonate(btn) {
        ...
        // Simulate success
        const successText = this.scene.add.text(0, 150, 'DONATION SENT!', {
            fontFamily: '"Press Start 2P"',
            fontSize: '16px',
            fill: '#00ff00',
            stroke: '#000000',
