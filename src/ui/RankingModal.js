...
export default class RankingModal extends UIModal {
    constructor(scene) {
        super(scene, 460, 600, LanguageManager.get('ranking_title', {}, 'LEADERBOARD'));
        this.populate();
    }
    ...
    renderList(ranking) {
        ...
        // Headers
        this.createRow(0, startY - 30, width, rowHeight, { rank: '#', name: 'PLAYER', score: 'SCORE' }, true);

        ranking.forEach((entry, index) => {
            const y = startY + index * rowHeight;
            // Limit to top 10 to fit
            if (index < 10) {
                this.createRow(0, y, width, rowHeight, {
    ...
    createRow(x, y, w, h, data, isHeader, index = -1) {
        ...
        if (!isHeader) {
            ...
            let strokeAlpha = 0;
            if (index === 0) { strokeColor = 0xffd700; strokeAlpha = 1; } // Gold
            else if (index === 1) { strokeColor = 0xc0c0c0; strokeAlpha = 1; } // Silver
            else if (index === 2) { strokeColor = 0xcd7f32; strokeAlpha = 1; } // Bronze
        ...
        // Rank
        const rankText = this.scene.add.text(-w / 2 + 30, 0, data.rank, style).setOrigin(0.5);

        // Name (Left Aligned)
        const nameText = this.scene.add.text(-w / 2 + 80, 0, this.truncateName(data.name), style).setOrigin(0, 0.5);

        // Score (Right Aligned)
        const scoreText = this.scene.add.text(w / 2 - 30, 0, data.score, { ...style, fill: '#ffff00' }).setOrigin(1, 0.5);
