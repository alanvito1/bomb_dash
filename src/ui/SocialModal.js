...
export default class SocialModal extends UIModal {
    ...
    createTabs() {
        ...
        tabs.forEach((tab, index) => {
            ...
            const btn = this.scene.add.text(startX + (index * spacing), y, tab.label, {
    ...
    createGuildRow(x, y, guild) {
        ...
        // Tag
        const tag = this.scene.add.text(-w/2 + 20, 0, `[${guild.tag}]`, {
             fontFamily: '"Press Start 2P"', fontSize: '12px', fill: '#ffd700'
        }).setOrigin(0, 0.5);

        // Name
        const name = this.scene.add.text(-w/2 + 100, 0, guild.name, {
             fontFamily: '"Press Start 2P"', fontSize: '10px', fill: '#fff'
        }).setOrigin(0, 0.5);

        // Members
        const members = this.scene.add.text(w/2 - 80, 0, `${guild.memberCount} Mbrs`, {
             fontFamily: '"Press Start 2P"', fontSize: '8px', fill: '#888'
        }).setOrigin(1, 0.5);

        // Join Button (if not in guild)
        if (!this.myGuild) {
            const btn = this.createButton(w/2 - 40, 0, 'JOIN', 60, 24, () => this.joinGuild(guild.id));
            container.add(btn);
        }
    ...
    createButton(x, y, label, w, h, callback) {
        ...
        const text = this.scene.add.text(0, 0, label, {
             fontFamily: '"Press Start 2P"', fontSize: '10px', fill: '#000'
        }).setOrigin(0.5);
