...
test.describe('E2E: PvP Wager De-Leveling Flow', () => {
  ...
  test('a player should lose XP, de-level, and see the correct UI state', async ({
    page,
  }) => {
    // 1. Go to the game and set the initial state by simulating a login.
    await page.goto('/');
    await page.waitForFunction(
      () => window.game && window.game.isBooted,
      null,
      { timeout: 10000 }
    );
    ...
    await page.evaluate((user) => {
      window.game.scene
        .getScene('MenuScene')
        .scene.start('ProfileScene', { userData: user });
    }, loserData.user);

    await page.waitForFunction(() =>
      window.game.scene.isActive('ProfileScene')
    );
