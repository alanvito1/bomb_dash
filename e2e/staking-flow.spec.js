...
const HERO_STAKING_ADDRESS =
  process.env.HERO_STAKING_ADDRESS ||
  '0x9A9f2CCa3556499352751C54e1955120d6594A49';
const MOCK_HERO_NFT_ADDRESS =
  process.env.MOCK_HERO_NFT_ADDRESS ||
  '0x09635F643e140090A9A8D5F77053692B45011B18';

test.describe('NFT Hero Staking Flow', () => {
  test('should allow a user to approve and deposit an NFT hero to play', async ({
    page,
  }) => {
    // --- 1. Setup and Mocking ---
    await page.goto('/');
    ...
    await page.route('**/api/heroes', (route) => {
      ...
      console.log(
        `MOCK API: /api/heroes called. Responding with hero status: ${hero.status}`
      );
      route.fulfill({
        status: 200,
    ...
    await expect(
      page.getByRole('button', { name: 'Modo Aventura' })
    ).toBeVisible();
    ...
    const depositButton = heroCard.getByRole('button', {
      ...
      async (addresses) => {
        ...
        const mockContract = {
          setApprovalForAll: () => ({
            send: async () => {
              console.log(
                'MOCK Contract: setApprovalForAll called and resolved.'
              );
              return {
                transactionHash: '0x' + 'b'.repeat(64),
                wait: async () => console.log('Approval TX waited'),
              };
            },
          ...
          depositHero: () => ({
            send: async () => {
              ...
              return {
                ...
                wait: async () => console.log('Deposit TX waited'),
              };
            },
            estimateGas: async () => 100000n,
          }),
        ...
        window.stakingService.approve = async () => {
          ...
          const tx = {
            wait: async () => {
              console.log('Mock approve tx mined');
              return { status: 1 };
            },
          };
        ...
        window.stakingService.depositHero = async (tokenId) => {
          ...
          const tx = {
            wait: async () => {
              console.log('Mock deposit tx mined');
              return { status: 1 };
            },
          };
          return Promise.resolve(tx);
    ...
    await expect(heroCard.getByText('APPROVING...')).toBeVisible({
      timeout: 5000,
    });
    await expect(heroCard.getByText('DEPOSITING...')).toBeVisible({
      timeout: 10000,
    });
    ...
    await expect(heroCard.getByText('Status: Staked')).toBeVisible({
      timeout: 10000,
    });

    // The "Level Up" button should now be visible instead of the deposit button
    await expect(
      heroCard.getByRole('button', { name: 'LEVEL UP' })
    ).toBeVisible();
