...
test.describe('1v1 Ranked PvP Flow', () => {
  test('should complete a full 1v1 ranked match flow with Sunday bonus', async ({
    page,
  }) => {
    // --- 1. Setup and Mocking ---
    await page.goto('/');
        ...
        if (url.includes('/matchmaking/status')) {
          ...
          return Promise.resolve({
            ...
            match: {
              ...
              opponent: {
                userId: 999,
                hero: { id: 999, sprite_name: 'Witch', level: 5 },
              },
            },
          });
        }
      ...
      window.api.post = async (url, data) => {
        ...
        if (url.includes('/pvp/ranked/report')) {
          ...
          return Promise.resolve({
            success: true,
            message:
              'Partida finalizada! Você ganhou 55 XP para o herói e 22 XP para a conta. (Bônus de Domingo de +10% aplicado!)',
            rewards: { heroXp: 55, accountXp: 22, bonusApplied: true },
          });
        }
        return Promise.resolve({
          success: false,
          message: 'Unhandled POST mock',
        });
      };
    });
    ...
    await expect(
      page.getByRole('button', { name: 'PvP Ranqueado' })
    ).toBeVisible();
    ...
    await expect(
      page.getByRole('button', { name: 'Entrar na Fila (Taxa: 10 BCOIN)' })
    ).toBeVisible();
        ...
        constructor(abi, address) {
          ...
          funcs.forEach((func) => {
            this.methods[func.name] = (...args) => ({
              send: async ({ from }) => {
                console.log(
                  `Mocked contract call: ${func.name} from ${from} with args`,
                  args
                );
    ...
    await page
      .getByRole('button', { name: 'Entrar na Fila (Taxa: 10 BCOIN)' })
      .click();

    // --- 4. Verify Matchmaking Flow ---
    await expect(
      page.getByText('Você está na fila! Procurando oponente...')
    ...
    await expect(
      page.getByText(/Você ganhou 55 XP para o herói/)
    ).toBeVisible();
    await expect(
      page.getByText(/\(Bônus de Domingo de \+10% aplicado!\)/)
    ).toBeVisible();

    console.log(
      'Test completed successfully: Full PvP flow with Sunday bonus validated.'
    );
  });
});
