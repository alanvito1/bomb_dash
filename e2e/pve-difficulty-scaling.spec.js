...
async function setupAndStartSoloGame(page, accountLevel) {
    ...
    await page.evaluate(() => localStorage.setItem('jwtToken', 'dummy-test-token'));

    // Now, navigate to the app. It will find the token on its initial load.
    await page.goto('/');

    // Wait for i18n to be ready to avoid race conditions
    await page.waitForFunction(() => window.i18nReady === true, { timeout: 15000 });
    ...
    await page.waitForFunction(() => window.game.scene.isActive('GameScene'), { timeout: 10000 });
}

test.describe('PvE Dynamic Difficulty Scaling', () => {

    test('should spawn enemies with baseline stats for a Level 1 account', async ({ page }) => {
    ...
    test('should spawn enemies with scaled stats for a Level 10 account', async ({ page }) => {
