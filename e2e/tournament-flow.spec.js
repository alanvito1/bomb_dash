...
test.describe('Multi-Player Tournament Flow', () => {
  ...
  test('should allow a player to navigate to the tournament lobby', async ({
    page,
  }) => {
    ...
    await page.evaluate(() => {
      ...
      const tournamentsButton = menuScene.children.list.find(
        (c) => c.name === 'tournament_button'
      );
    ...
    await page.waitForFunction(() =>
      window.game.scene.isActive('TournamentLobbyScene')
    );

    const isLobbyActive = await page.evaluate(() =>
      window.game.scene.getScene('TournamentLobbyScene').scene.isActive()
    );
