...
test.describe('Comprehensive Bomb Dash E2E Tests', () => {
  ...
  test('1.1 - Web3 Authentication (SIWE)', async ({ page }) => {
    await login(page);
    const onMenuScene = await page.evaluate(() =>
  ...
  test('1.3 - Progression and Upgrades (XP Gain and Level Up)', async ({
    page,
  }) => {
    await login(page);
    ...
    await page.route('**/api/solo/report', (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
          success: true,
    ...
    await page.route(`**/api/heroes/${heroId}/level-up`, (route) =>
      route.fulfill({
        status: 200,
  ...
  test('2.1 - Solo Reward System (Claim Flow)', async ({ page }) => {
    ...
    await page.route('**/api/solo/claim-reward', (route) =>
      route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({
    ...
    const notificationText = await page.evaluate(() => {
      ...
      return scene.children.list.find((c) => c.name === 'notificationText')
        .text;
    });
  ...
  test('2.2 - PvP 1v1 (Ranked Fee Flow)', async ({ page }) => {
    ...
    await page.route('**/api/pvp/ranked/enter', (route) =>
      route.fulfill({
        status: 200,
  ...
  test('2.3 - Hero Staking (Deposit and Withdraw)', async ({ page }) => {
    ...
    await page.route(`**/api/heroes/${heroId}/stake`, (route) =>
      route.fulfill({
        status: 200,
    ...
    await page.route(`**/api/heroes/${heroId}/initiate-withdrawal`, (route) =>
      route.fulfill({
        status: 200,
    ...
    await page.evaluate(
      (id) =>
        window.game.scene
          .getScene('HeroSelectionScene')
          .heroCards.find((c) => c.heroData.id === id)
          .emit('stakeRequest'),
      heroId
    );

    // Verify the UI updates to show the hero is staked.
    let heroStatus = await page.evaluate(
      (id) =>
        window.game.scene
          .getScene('HeroSelectionScene')
          .heroCards.find((c) => c.heroData.id === id).heroData.status,
      heroId
    );
    ...
    heroStatus = await page.evaluate(
      (id) =>
        window.game.scene
          .getScene('HeroSelectionScene')
          .heroCards.find((c) => c.heroData.id === id).heroData.status,
  ...
  test('2.4 - Altar of Global Buffs (Donation Flow)', async ({ page }) => {
    ...
    await page.route('**/api/altar/status', (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
          success: true,
    ...
    await page.route('**/api/altar/donate', (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
          success: true,
    ...
    const notificationText = await page.evaluate(
      () =>
        window.game.scene
          .getScene('NotificationScene')
          .children.list.find((c) => c.name === 'notificationText').text
    );
  ...
  test('2.5 - Player Ranking System (View Rankings)', async ({ page }) => {
      ...
      route.fulfill({
        ...
        body: JSON.stringify({
          ...
          ranking: [
            {
              rank: 1,
              wallet_address: '0xPlayerOne...',
              highest_wave_reached: 50,
            },
            {
              rank: 2,
              wallet_address: '0xPlayerTwo...',
              highest_wave_reached: 45,
            },
          ],
        }),
      })
    ...
    const rankingText = await page.evaluate(() => {
      ...
      return scene.children.list.find((c) => c.name === 'rankingDisplayText')
        .text;
    });
  ...
  test('2.6 - PvP XP Wager System (De-level Scenario)', async ({ page }) => {
    ...
    await page.route('**/api/pvp/wager/report', (route) =>
      route.fulfill({
        status: 200,
        body: JSON.stringify({
          success: true,
    ...
    const finalHeroLevel = await page.evaluate((id) => {
      return window.game.scene
        .getScene('HeroSelectionScene')
        .heroCards.find((c) => c.heroData.id === id).heroData.level;
    }, heroId);

    const finalHeroXp = await page.evaluate((id) => {
      return window.game.scene
        .getScene('HeroSelectionScene')
        .heroCards.find((c) => c.heroData.id === id).heroData.xp;
    }, heroId);
  ...
  test('3.1 - Multi-Player Tournament (Graceful Failure)', async ({ page }) => {
    ...
    await page.route('**/api/tournaments/join', (route) =>
      route.fulfill({
    ...
    await page.evaluate(() =>
      window.game.scene.getScene('NotificationScene').dismiss()
    );
    const isPvpSceneStillActive = await page.evaluate(() =>
      window.game.scene.isActive('PvpSelectionScene')
    );
  ...
  test('4.1 - Full User Lifecycle Simulation', async ({ page }) => {
    ...
    await page.route('**/api/heroes/1/stake', (route) =>
      route.fulfill({ status: 200, body: '{"success":true}' })
    );
    await page.route('**/api/altar/donate', (route) =>
      route.fulfill({ status: 200, body: '{"success":true}' })
    ...
    await page.route('**/api/ranking', (route) =>
      route.fulfill({ status: 200, body: '{"success":true, "ranking":[]}' })
    );
    ...
    await waitForScene(page, 'AuthChoiceScene');
    const onAuthScene = await page.evaluate(() =>
      window.game.scene.isActive('AuthChoiceScene')
    );
