...
test.describe('Hero Progression End-to-End Flow', () => {
    ...
    test('should allow a user to level up a hero and see the UI update', async ({ page }) => {
        ...
        await page.evaluate((updatedHero) => {
            window.api.levelUpHero = async (heroId) => {
                console.log(`E2E MOCK: window.api.levelUpHero called with heroId: ${heroId}`);
        ...
        const heroCard = page.locator('.phaser-game').getByText(MOCK_HERO_NAME).locator('..');
        ...
        await expect(page.locator('.phaser-game').getByText('Success!')).toBeVisible();
        await expect(page.locator('.phaser-game').getByText(`${MOCK_HERO_NAME} is now Level ${MOCK_HERO_AFTER_LEVEL_UP.level}!`)).toBeVisible();
        ...
        await expect(heroCard.getByText(`Lvl: ${MOCK_HERO_AFTER_LEVEL_UP.level}`)).toBeVisible();
