...
test.describe('E2E: BCOIN Balance Display with Network Mocking', () => {
  test('should display mocked BCOIN balance by intercepting RPC calls', async ({
    page,
  }) => {
    ...
    await page.route('**/*', async (route) => {
      ...
      if (
        ...
        postData.params[0]?.data?.startsWith('0x70a08231')
      ) {
        return route.fulfill({
          json: {
            id: postData.id,
    ...
    await page.waitForFunction(() => window.game?.isBooted, null, {
      timeout: 15000,
    });
